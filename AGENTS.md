...
5. **POORTWACHTER (server-side, fail-closed)** — after every build/qc run the SERVER itself verifies the evidence: craft-lint passes + `jury.eind` all ≥ 8 + `qc.pass` true. Verdict lands in `status.json.gate`; without a passing gate the run is registered as FAILED, the dashboard shows "Concept — nog niet goedgekeurd" and the stage can never move to sent/live. The AI never approves its own work — typing "done" without evidence does nothing.
...
- **Every client site is a STAGE (toneel):** ONE viewport height, pinned. Scrolling goes DOWN (natural thumb gesture — Jay definitief 2026-07-03: nooit zijwaarts), and the scenes change INSIDE the fixed frame as you scroll: each scene cinematically hands over to the next (world slides/fades up, new world rises — filmic transitions, not hard swaps), snap on scene-COMPLETE. Progress dots. NEVER endless stacked sections.
